...
(async () => {
	...
	await binCheck(systemBin, ['-version']).catch(error => {
		log.warn(`The \`${systemBin}\` binary doesn't seem to work correctly`);
		throw error;
	});
	const version = '>=3.0';
	await binVersionCheck(systemBin, version, {args: ['-version']}).catch(error => {
		log.warn(`The \`${systemBin}\` binary doesn't satisfy version \`${version}\``);
		throw error;
	});
